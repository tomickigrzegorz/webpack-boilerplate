...
(function (global, Factory) {
  ...
  this,
  /** @constructor */ function SmoothscrollAnchorPolyfill() {
    var instance = this,
    ...
    function findInParents(element, validate) {
      ...
      if (element.parentElement)
        return findInParents(element.parentElement, validate);
      return null;
    }
    ...
    function triggerSmoothscroll(target) {
      ...
      if (supportsPreventScroll) focusElement(target);
        ...
        pendingFocusChange = setTimeout(focusElement.bind(null, target), 450);
    }
    ...
    function handleClick(evt) {
      ...
      var anchor = /** @type {?HTMLAnchorElement} */ (findInParents(
        getEventTarget(evt),
        isAnchorToLocalElement
      ));
      if (!anchor) return;
